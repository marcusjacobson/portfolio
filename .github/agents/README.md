...
| [`@project-intake`](project-intake.agent.md) | Roadmap-shaped variant of intake: drafts a project/roadmap item with Pillar, Tier, Goal, and Next steps; creates it as a DraftIssue on board #13 (no repo issue is filed) and sets Status=Todo plus Pillar/Tier/Priority. The `Project` label only applies post-conversion if the draft is promoted to a real issue. | Read-only until approval |
