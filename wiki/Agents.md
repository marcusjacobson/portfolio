...
| [`@project-intake`](https://github.com/marcusjacobson/portfolio/blob/main/.github/agents/project-intake.agent.md) | Roadmap-shaped variant of intake: files a real GitHub issue with `project` + `needs-triage` labels (auto-routed onto board #13 Security Portfolio Roadmap and board #19 Triage Queue), then sets `Status=Hold` on board #13 so new ideas park until triage clears them. After triage and confirmation, [`@projects-publishing`](https://github.com/marcusjacobson/portfolio/blob/main/.github/agents/projects-publishing.agent.md) lands the corresponding card on `ms_security_projects.html`. | User proposes a new portfolio project, capstone, or roadmap initiative. | `@triage` (clears `needs-triage`), then `@issue-resolver` for scoped deliverables, then `@projects-publishing` for the public card. | `no` | No — issue filing requires interactive classification and explicit approval; cloud agent isn't engaged at intake. |
| [`@cert-intake`](https://github.com/marcusjacobson/portfolio/blob/main/.github/agents/cert-intake.agent.md) | Cert-shaped variant of intake: researches vendor exam requirements (skills outline, prerequisites, retirement notices), drafts a real issue, and files it with the `certification` label so the tag-routing workflow places it on the Certification/Education Path board (#2). Sets the board's Vendor, Exam Code, Start, and End fields after auto-add fires. | User proposes a new certification or education-path goal — "study for SC-401", "track CompTIA CySA+", "plan AZ-500 → SC-500". | None until study deliverables are scoped; then `@issue-resolver`. | `no` | Yes — once an exam is scoped into concrete study deliverables, those sub-issues are good cloud-agent candidates. |
| [`@issue-resolver`](https://github.com/marcusjacobson/portfolio/blob/main/.github/agents/issue-resolver.agent.md) | Resolves a single GitHub issue end-to-end: branch, implement, lint, commit, open PR, watch checks, merge. Requires an explicit issue number. Cloud-hardened with a fail-fast loop (≤2 attempts at the same approach before posting a blocker comment) and a hard ban on `gh pr merge --admin` so it never bypasses required checks. | One issue is fully scoped and ready to ship. | `@security-reviewer` (PR review) or `@publish-manager` (publish flow). | `yes` | Alternative — same job as the hosted agent, but synchronous and human-in-the-loop. Pick local for orchestration-heavy issues, hosted for one-shot file edits. |
...
| [`@triage`](https://github.com/marcusjacobson/portfolio/blob/main/.github/agents/triage.agent.md) | Triages issues currently labeled `needs-triage` — confirms them for work (labels + priority + board placement) or dismisses them (close as wontfix/duplicate). Operates one issue at a time in local Copilot Chat, and removes the issue from the Triage Queue board (#19) on each disposition so the queue stays a true backlog. | The `needs-triage` queue has open items, or a maturity-scout / external-scanner issue lands and needs disposition. | `@issue-resolver` (single confirmed item) or `@boards-worker` (batch drain after confirmations). | `yes` | No — chat-only. |
...
| [`@repo-cleanup`](https://github.com/marcusjacobson/portfolio/blob/main/.github/agents/repo-cleanup.agent.md) | Sweeps the repo for incidental work artefacts and walks **one candidate per turn**, pausing for explicit `delete` / `keep` / `cancel` before moving on. Bakes the issue+branch+PR workflow into the loop so deletions land through the standard merge gate, and `keep` writes a remediation (`.gitignore`, `.cleanupignore`, or in-file marker) so the same item isn't re-flagged. | Periodic hygiene sweep, after long Copilot sessions, or when `staging-inbox/` / `test-results/` / `playwright-report/` has accumulated. | None — terminal step; each candidate's removal lands as its own PR. | `no` | No — local-only file scan and remediation; the hosted agent doesn't see uncommitted state. |
...
| Triage / intake (turning a chat ask into an issue) | Local (`@request-intake`, `@bug-intake`, `@project-intake`, `@cert-intake`) | Interactive classification, dedupe checks, and approval before any mutation. |
