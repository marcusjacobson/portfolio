...
| [`/wiki-sync-run`](https://github.com/marcusjacobson/portfolio/blob/main/.github/prompts/wiki-sync-run.prompt.md) | `agent` | End-to-end wiki-sync run: detect repo↔wiki drift via [`@wiki-sync`](Agents), file each delta through [`@request-intake`](Agents), sweep onto board #16 via [`@board-planner`](Agents), and advance the state cursor only when the whole batch resolves. | Optional flags forwarded to `@wiki-sync` (e.g. `--paths .github/agents/** --max=5 --since=<sha>`). | [`@wiki-sync`](Agents) for detection, then [`@request-intake`](Agents) per delta, then [`@board-planner`](Agents) for the board #16 sweep. |
