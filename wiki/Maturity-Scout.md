...
| `source:github-docs` | `maturity-scan.yml` steps `scan_github_docs`, `scan_permissions_block`, `scan_codeowners_shape`, `scan_reusable_workflow_pin` | 4 rules: action SHA-pin audit across `.github/workflows/*.yml`; top-level `permissions:` block presence on every workflow (#257); CODEOWNERS shape validation when the file exists — every non-comment, non-blank line must match `<glob> @<owner>[ @<owner>...]` (#257); reusable-workflow SHA pin audit — every `<owner>/<repo>/.github/workflows/<wf>.yml@<ref>` ref must end in a 40-char commit SHA (#257). Each rule files at most one consolidated issue per run and shares the run-wide MAX cap. | Branch-protection doc completeness (does the wiki match what's enforced?); CODEOWNERS overlapping-glob shadow detection; concurrency block hygiene; `pull_request_target` usage audit. | **Expanded** in #257. Remaining gaps tracked separately if/when they become a priority. |
...
- `source:github-docs` covers four rules: (1) action SHA-pin audit across every `.github/workflows/*.yml` file; (2) top-level `permissions:` block presence on every workflow (added in #257); (3) CODEOWNERS shape validation when the file exists — every non-comment, non-blank line must match `<glob> @<owner>[ @<owner>...]` (added in #257); (4) reusable-workflow SHA pin audit — every `<owner>/<repo>/.github/workflows/<wf>.yml@<ref>` ref must end in a 40-char commit SHA (added in #257). Each rule files **one consolidated issue** per run and shares the run-wide MAX cap with the rest of the chain. Remaining github-docs gaps (branch-protection doc completeness, CODEOWNERS overlapping-glob shadow detection, concurrency block hygiene, `pull_request_target` usage audit) stay on-demand under `@maturity-scout`.
