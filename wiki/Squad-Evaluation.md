...
| `squad-issue-assign.yml` (label-routes `squad:<member>` to a member) | Existing routing through intake agents + `@copilot` assignment + [`triage-route.yml`](https://github.com/marcusjacobson/portfolio/blob/main/.github/workflows/triage-route.yml) | **Adds an 11-workflow surface** that needs audit, pinning, `permissions:` review, and conflict-checking against existing label-routing. |
